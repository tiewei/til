...
func (*AWSSQS) Manifests(id string, config, eventDst cty.Value, _ globals.Accessor) []interface{} {
	...
	if !eventDst.IsNull() {
		...
		subs := k8s.NewSubscription(name, name, k8s.NewDestination(k8s.APITargets, "AWSSQSTarget", name), eventDst)
...
func (*AWSSQS) Address(id string, _, eventDst cty.Value, _ globals.Accessor) cty.Value {
	name := k8s.RFC1123Name(id)
