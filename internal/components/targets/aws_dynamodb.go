...
var (
	...
	_ translation.Addressable  = (*AWSDynamoDB)(nil)
)
...
func (*AWSDynamoDB) Manifests(id string, config, eventDst cty.Value, glb globals.Accessor) []interface{} {
	var manifests []interface{}

	name := k8s.RFC1123Name(id)

	t := k8s.NewObject(k8s.APITargets, "AWSDynamoDBTarget", name)
	...
	if !eventDst.IsNull() {
		...
		subscriber := k8s.NewDestination(k8s.APITargets, "AWSDynamoDBTarget", name)
...
func (*AWSDynamoDB) Address(id string, _, eventDst cty.Value) cty.Value {
	name := k8s.RFC1123Name(id)

	if eventDst.IsNull() {
		return k8s.NewDestination(k8s.APITargets, "AWSDynamoDBTarget", name)
	}
